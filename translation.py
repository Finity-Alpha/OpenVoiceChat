...
class Translator:
    ...
    @torch.no_grad()
    def say(self, text):
        translated_text, wav, sr = self.model.predict(text, "t2st", 'urd', src_lang='eng')
...
if __name__ == "__main__":
    translator = Translator_m4t("seamlessM4T_large", vocoder_name_or_card="vocoder_36langs", device=torch.device("cuda:0"))
    audio = record(3)
